...
class MLPY_SVM(object):
  def __init__(self, method_param, run_param):
    self.info = f"MLPY_SVM ({str(method_param)})"
    ...
    self.build_opts = {
        "kernel_type":
        method_param["kernel"] if "kernel" in method_param else 'rbf',
        "C": float(method_param["c"]) if "c" in method_param else 1.0,
        "gamma":
        float(method_param["gamma"]) if "gamma" in method_param else 0.0,
    }
